...
const HeroAnimated = () => (
  <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 p-8 md:p-16 text-white">
    ...
    <div className="relative z-10 max-w-4xl">
      <p initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mb-3 text-sm uppercase tracking-widest text-cyan-200">
        Career Platform 2.0
      </p>
      <h1 initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="text-4xl md:text-6xl font-bold leading-tight">
      ...
      <p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mt-6 text-slate-200 text-lg md:text-xl">
        Explore modern roles, AI-powered recommendations, and a guided hiring journey designed for high-impact engineers.
      </p>

      <div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="mt-8 flex flex-col sm:flex-row gap-4">
      ...
      <div className="mt-10 grid grid-cols-1 sm:grid-cols-3 gap-4">
        {stats.map((item, index) => (
          <div key={item.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 + index * 0.1 }} className="rounded-xl border border-white/10 bg-white/5 p-4 backdrop-blur">
